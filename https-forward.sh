...
if [ -z "$HTTPS_TARGET" ]; then
  echo "ERROR: HTTPS_TARGET environment variable not set"
  echo "Please set using: -e HTTPS_TARGET=<target_ip>"
  exit 1
fi
...
echo "Forwarding HTTPS traffic to: $HTTPS_TARGET"

# 启动socat进行端口转发
exec socat \
    TCP-LISTEN:${HTTPS_PORT:-443},fork,reuseaddr \
    TCP:$HTTPS_TARGET:443,bind=$WG_IP